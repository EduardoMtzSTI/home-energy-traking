package com.marman.api_gateway.route;
...
public class IngestionServiceRoutes {
    ...
    @Bean
    public RouterFunction<ServerResponse> igestionRoute() {
    ...
    @Bean
    public RouterFunction<ServerResponse> igestionFallbackRoute() {
        return route("fallbackRoute")
                .route(RequestPredicates.path("/fallbackRoute"),
                        request-> ServerResponse.status(HttpStatus.SERVICE_UNAVAILABLE)
                                .body("Igestion service is down"))
                .build();
    }
